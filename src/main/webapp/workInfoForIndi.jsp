...
<body>
	...
	<div>
		...
		<%}else{for(int i = 0 ; i < list.size(); i++){%>
			<p onclick="getAllWorkInfo('<%=list.get(i).getWorkCode()%>');"><%=i+1 %>. 현장명 : <%=list.get(i).getWorkField() %> | 현장 책임자 : <%=list.get(i).getFieldManager() %> | 현장 주소 : <%=list.get(i).getFieldAdd01() %> | 현장 상세 주소 : <%=list.get(i).getFieldAdd02() %> | 상태 : <%=status[i] %></p>
		<%}} %>
	</div>
